...
const ActionButtons: React.FC = () => {
  ...
  return (
    <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'row', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>

      {/* Hint */}
      {status === 'playing' && !showSolution && (
        <button onClick={getHint} className="btn-secondary">
          <span style={{ fontSize: 16, marginRight: 6 }}>💡</span>
          <span style={{ color: '#f0b429', fontWeight: 700, fontSize: 14 }}>Pista</span>
        </button>
      )}
      ...
      {status === 'playing' && (
        <button onClick={autoSolve} className="btn-secondary">
          <span style={{ fontSize: 16, marginRight: 6 }}>⏩</span>
          <span style={{ color: '#b5b1ab', fontWeight: 700, fontSize: 14 }}>Resolver</span>
        </button>
      )}
      ...
      {status === 'playing' && wrongCount >= 2 && !showSolution && (
        <button onClick={viewSolution} className="btn-secondary">
          ...
          <span style={{ color: '#b5b1ab', fontWeight: 700, fontSize: 14 }}>Solución</span>
        </button>
      )}
      ...
      {showSolution && status === 'playing' && currentPuzzle && (
        <div style={{
          ...
          <span style={{ color: '#f0b429', fontWeight: 700, fontSize: 16, fontFamily: 'monospace' }}>
      ...
      {status === 'completed' && (
        ...
        >
          <span style={{ color: '#fff', fontWeight: 700, fontSize: 16, marginRight: 6 }}>Siguiente Puzzle</span>
