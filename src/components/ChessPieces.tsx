...
// High-quality chess piece SVGs based on standard Merida/CBurnett style
  ...
  wR: ({ size }) => (
    <svg viewBox="0 0 45 45" width={size} height={size}>
      <g fill="#fff" stroke="#000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M14 29.5L11 32h23l-3-2.5H14z" strokeLinecap="butt"/>
        <path d="M11 32v2h23v-2H11z" strokeLinecap="butt" strokeLinejoin="miter"/>
      </g>
    </svg>
  ),
  wB: ({ size }) => (
...
export const ChessPiece: React.FC<{ piece: string; color: 'w' | 'b'; size: number }> = ({ piece, color, size }) => {
  const key = `${color}${piece.toUpperCase()}` as keyof typeof pieces;
  const PieceComponent = pieces[key];
  if (!PieceComponent) return null;
  return <PieceComponent size={size} />;
};
